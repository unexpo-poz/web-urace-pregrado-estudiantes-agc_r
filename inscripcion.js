...
function actualizarTotales(fp,ft,$update) {
    ...
    k = fp.CB.length - 1;
	i = fp.cola.length; // Modificado para contador de ultima asignatura en cola
    ...
    with(fp) {
	   ...
	   y = 0;
		while(y < i){
			...
			mat = cola[y].value.split(" ");               
			//alert(mat[5]);
			color  = mat[5];


			//if ((cola[y].selectedIndex != '0') && (color == '0')) { 
...
///////////////////////////////////////////////////////////////////		
		...
		//if (cod_uc[0] == '300677'){
			if ((materias.lastIndexOf("300676") != "-1") && (materias.lastIndexOf("300677") != "-1")){
				error=('ATENCION:\n\nLas asignaturas VENEZUELA EN EL SIGLO XXI y CIUDADANÍA Y PODER\n');
...
function calcularMaxCargaPOZ() {
    ...
    with (document.pensum) {
        for(j=0;j < (CB.length - 1); j++) {
			...
			//if ((vt_lapso !='I') && (CB[j].selectedIndex !='0')) {
//###############################################################################################
			if ((vt_lapso !='I')) { // Modificado para Inscripciones Lapso 2015-2. La repitencia afecta a todas las asignaturas.
...
function excesoDeCreditosPOZ(lSeccion) {
    ...
    if (matAinsc!=0 ){
     ...
     /*Añadido el 08/06/2018 para evitar inscripcion mayor a 3 materias
       limite establecido para el Lapso 2018-E*/
     /*if ( total_mat > 3){
        exceso = true;
        mens1 = "    PROBLEMA DE EXCESO DE ASIGNATURAS:\nNo puedes ";
        (ucm > 0) ? mAQ = "agregar" : mAQ = "borrar";
        mens1  = mens1 + mAQ + " esta asignatura.\n"
        mensLC = "3 materias.\n";
        mensCS = " y estas intentando inscribir " + total_mat + " materias.\n";       
        mcausa = "El límite para este Lapso es ";
    }*/
     /*FIN LIMITE 2018-E*/
...
function reiniciarTodo() {
    ...
    //RECORDAR: Se desactivo el sexo y fecha de las validaciones por cambios que se hicieron para que no validen o cambien el sexo los estudiantes - 01/06/2017
	/*
...
//Desactivar el campo de sexo para validaciones, por problemas de cambios por parte de los estudiantes-cambios efectuados 01/06/2017
	...
	with (document.pensum) {
		for(j=0;j < (cola.length); j++){
			...
			if (cola[j].selectedIndex != '0') {
				...
				temp = parseInt(cod_uc[9]);	

				if (temp>10) { // Si es electiva
					temp = temp - 3; 
				}
				...
				if (temp < semBajo) {
					...
					//alert ('temp='+temp+' < semBajo '+semBajo);
					...
					for(j=0;j < (cola.length); j++){
						...
						if ((cola[j].selectedIndex == '0') || (status != 'B')) {
							cod_uc = cola[j].value.split(" ");
							//alert(cola[j].value+'-'+cod_uc[7]);
